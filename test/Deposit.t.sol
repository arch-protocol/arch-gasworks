pragma solidity ^0.8.13;

import {Test} from "forge-std/Test.sol";
import "forge-std/console2.sol";

import "../src/Deposit.sol";
import {SigUtils} from "./utils/SigUtils.sol";
import "solmate/tokens/ERC20.sol";
import "./utils/HexUtils.sol";

contract DepositTest is Test {
    ///                                                          ///
    ///                           SETUP                          ///
    ///                                                          ///

    Deposit internal deposit;
    ERC20 internal usdc;
    ERC20 internal web3;
    SigUtils internal sigUtils;

    uint256 internal ownerPrivateKey;
    address internal owner;

    function setUp() public {
        usdc = ERC20(0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174);
        web3 = ERC20(0xBcD2C5C78000504EFBC1cE6489dfcaC71835406A);
        deposit = new Deposit();
        sigUtils = new SigUtils(usdc.DOMAIN_SEPARATOR());

        ownerPrivateKey = 0xA11CE;
        owner = vm.addr(ownerPrivateKey);

        vm.prank(0xF977814e90dA44bFA03b6295A0616a897441aceC);
        usdc.transfer(owner, 10e6);

        vm.deal(address(deposit), 10 ether);
    }

    ///                                                          ///
    ///                           DEPOSIT                        ///
    ///                                                          ///

    function test_Deposit() public {
        vm.prank(owner);
        usdc.approve(address(deposit), 10e6);

        string[] memory inputs = new string[](3);
        inputs[0] = "node";
        inputs[1] = "scripts/fetch-quote.js";
        inputs[2] = Conversor.iToHex(abi.encode(10e6));
        bytes memory res = vm.ffi(inputs);
        (address spender, address payable swapTarget, bytes memory quote, uint256 value) = abi.decode(res, (address, address, bytes, uint256));

        vm.prank(owner);
        deposit.deposit(address(usdc), 10e6, spender, swapTarget, quote, value);

        assertEq(usdc.balanceOf(owner), 0);
        assertEq(usdc.balanceOf(address(deposit)), 0);
        assertGe(web3.balanceOf(owner), 6e18);
    }   

    // function testFail_ContractNotApproved() public {
    //     vm.prank(owner);
    //     deposit.deposit(address(usdc), 1e6);
    // }

    ///                                                          ///
    ///                       DEPOSIT w/ PERMIT                  ///
    ///                                                          ///

    function test_DepositWithLimitedPermit() public {
        SigUtils.Permit memory permit = SigUtils.Permit({
            owner: owner,
            spender: address(deposit),
            value: 10e6,
            nonce: usdc.nonces(owner),
            deadline: 2**256 - 1
        });

        bytes32 digest = sigUtils.getTypedDataHash(permit);

        (uint8 v, bytes32 r, bytes32 s) = vm.sign(ownerPrivateKey, digest);

        // string[] memory inputs = new string[](3);
        // inputs[0] = "node";
        // inputs[1] = "scripts/fetch-quote.js";
        // inputs[2] = Conversor.iToHex(abi.encode(10e6));
        // bytes memory res = vm.ffi(inputs);
        // (address spender, address payable swapTarget, bytes memory quote, uint256 value) = abi.decode(res, (address, address, bytes, uint256));

        deposit.depositWithPermit(
            address(usdc),
            10e6,
            permit.owner,
            permit.spender,
            permit.value,
            permit.deadline,
            v,
            r,
            s
        );

        assertEq(usdc.balanceOf(owner), 0);
        assertEq(usdc.balanceOf(address(deposit)), 0);

        assertEq(usdc.allowance(owner, address(deposit)), 0);
        assertEq(usdc.nonces(owner), 1);
    }
}
